...
class PeftMixedModel(PushToHubMixin, torch.nn.Module):
    ...
    def __init__(self, model: nn.Module, peft_config: PeftConfig, adapter_name: str = "default") -> None:
        super().__init__()
        if MixedModel is None:
            raise ImportError(
                "PeftMixedModel requires the 'MixedModel' class which is not available in this version of PEFT. "
                "This custom FedALT PEFT build does not support mixed adapter types."
            )
